...
for stapSSi= 1:    size(ColorCode,1)    % fill ds part
    ...
    while sum(Next==[-1,-1])~=2
        ...
        BelongTransM(stapAndSacf)=ColorIndex;
%         if abs(dat.vstrands{CylIndNext}.stap(BaseNext+1,4)-Next(2))>30
%             dat.vstrands{CylIndNext}.stap(BaseNext+1,3:4)
%             sdfsf=123;
%         end
   ...
   FirstStage = sum(BelongTransM~=0)

 %%%%5---------------debug  
%    figure(222);clf;hold on;
%    for kkk=1:max(BelongTransM)
%        scatter3(T(BelongTransM==kkk,1),T(BelongTransM==kkk,2),T(BelongTransM==kkk,3),'.') ;
%    end
  %------------ 
...
ScafStrandInds =find(a0==max(a0)) ;
% StapNoAss=  RemainNotAssign(RemainNotAssign>sum(TStrand==1)) ;  % old
% scaf only one
StapNoAss = intersect( RemainNotAssign, find(~ismember(TStrand,ScafStrandInds) ))   ; 
nn=1;
while ~isempty(StapNoAss)
    for k=1:length(StapNoAss)
        ...
        Cand1BaseBC=  ConvT_CylBase(Cand1,:);
        if Cand2~=0
        Cand2BaseBC=  ConvT_CylBase(Cand2,:);
        else
         Cand2BaseBC=[0,100000];   
        end
        %--------new added June 21
        normD1=norm(Cand1BaseBC-ThisStapBaseBC);
        normD2=norm(Cand2BaseBC-ThisStapBaseBC);
        if normD1<normD2 && abs(ThisStapBaseBC(2)-Cand1BaseBC(2))<20 
        BelongTransM(StapNoAss(k))=  BelongTransM(Cand1);
        elseif abs(ThisStapBaseBC(2)-Cand2BaseBC(2))<20 
...
ScafNoAss= intersect( RemainNotAssign, find(ismember(TStrand,ScafStrandInds) ))   ; 
...
%              [qa2,qb2]= ismember(CylBaseBynumIndex, TwoSideConnec(1:2),'rows') ;
%              indLeft=find(qa2) ;
...
%              [qa,qb]= ismember(CylBaseBynumIndex, TwoSideConnec(3:4),'rows') ;
%              indRight=find(qa) ;
% %              if TwoSideConnec(3)~=CADNANOINDEX || abs(CylBase(2)-TwoSideConnec(4))~=1
% %                  indRight=0;
% %              end
%     Pooled =  union(indLeft, indRight) ;
%     NonQua= BelongTransM(Pooled) ==0 ;
%     Pooled=Pooled(~NonQua)  ;
%     if isempty(Pooled) 
%      continue 
%     end
%     Trust= CylBaseBynumIndex(Pooled(randi(length(Pooled))) ,:) ;
% 
% 
% %              if isempty(indLeft)  
% %                  if isempty(indRight)
...
% %                     Gindex(Gindex>sum(ismember(TStrand ,ScafStrandInds)))=[];
% 
%                     if  BelongTransM(Gindex)~=0
%                       AA=unique(BelongTransM(Gindex)   ) ;
%                       BelongTransM( ScafNoAss(kk))= AA(randi(length(AA))) ;
%                     else
%                         nothinghappen=1 ;
%                     end
%             end
...
%   BelongTransM(ScafNoAss)=3;
  for kkss=1:200
    ...
    for SSNi=1:length(ScafNoAss)
        ...
        %--------------add for polymerized staples
         ThisBC=  ConvT_CylBase( ScafNoAss(SSNi),:) ;
         ThisBCp1=  ConvT_CylBase( ScafNoAss(SSNi)+1,:) ;
         ThisBCn1=  ConvT_CylBase( ScafNoAss(SSNi)-1,:) ;
        
         S2_poly= abs(ThisBCp1(2)-ThisBC(2) )<30 ; % nt
         S1_poly= abs(ThisBCn1(2)-ThisBC(2) )<30  ;% nt
       
        %-----------
        
        
        if BelongTransM( ScafNoAss(SSNi)-1)~=0 && S1 && S1_poly
        BelongTransM(ScafNoAss(SSNi)) =  BelongTransM(ScafNoAss(SSNi)-1 );
        elseif  BelongTransM( ScafNoAss(SSNi)+1)~=0 && S2 && S2_poly
        BelongTransM(ScafNoAss(SSNi)) =  BelongTransM(ScafNoAss(SSNi)+1 ) ;
        end
    end
  end
...
%   
%   sdfsf=3
if  ~isempty(ScafNoAss) %still have unassigned
      for ssss=1:2
           for kk=1:length(ScafNoAss)
            ...
            RemainNotAssign=find(BelongTransM==0);
            ScafNoAss=  RemainNotAssign(RemainNotAssign<=sum(ismember(TStrand ,ScafStrandInds))) ;
...
%      %-------new in V5
    ...
    [C,IA] = uniquetol(BaseCentOnCly,tol,'ByRows',true,'OutputAllIndices',true);
    LengCell = cellfun(@length,IA) ;
    CellIA =IA(LengCell==2) ;
     ScafStapCorrelate=  cell2mat(CellIA')' ;
...
fprintf('finish first for loop in V5 searching forces. \n')
...
%     
    ...
    scatter3(T(ScafStapCorrelate2(RateCorrect==1,1),1),T(ScafStapCorrelate2(RateCorrect==1,1),2),T(ScafStapCorrelate2(RateCorrect==1,1),3),'.r') ;
    scatter3(T(ScafStapCorrelate2(RateCorrect==1,2),1),T(ScafStapCorrelate2(RateCorrect==1,2),2),T(ScafStapCorrelate2(RateCorrect==1,2),3),'.k') ;
...
%     end
    ...
    mmNewT=min(NewT(:,1:3)) ;
    MMNewT=max(NewT(:,1:3)) ;
    boxsize=floor(1.3*abs(mmNewT-MMNewT)/50)*50  ; 
...
%     scatter3(T(5169,1),T(5169,2),T(5169,3),'O')
