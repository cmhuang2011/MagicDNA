...
%--------Last editted , Oct 2nd 2018   % overhang json files contain
%mulitiple scaffold(1 true + N fakes)
...
for k=1:length(dat.vstrands)
    ...
    if sum(All)~=0
        ...
        ColRowStart= [ColRowStart;  ones(size(Base))*dat.vstrands{k}.col,  ones(size(Base))*dat.vstrands{k}.row]      ;
        
        %         Scaf_Start= [ClyIndx ,Base];            %v2
        %        ColRowStart= [dat.vstrands{k}.col,dat.vstrands{k}.row]      ;
        
        if   ScafR_Skip(1)==1  % only for real scaffold
            ScafR_Skip(1)=dat.vstrands{k}.skip(Base+1);
        end
        %        break;
    end
end
Scaf_Start=setdiff(Scaf_Start , [-1,-1] ,'stable','rows')   ;
ColRowStart=setdiff(ColRowStart , [-1,-1] ,'stable','rows')  ;



ScafR_AllBase=zeros( 10000,2); ks=2; ColRow=zeros( 10000,2);
ScafR_AllBase(1,:)=Scaf_Start(1,:);   ColRow(1,:)=ColRowStart(1,:);  % v3

Current=Scaf_Start(1,:) ; nth_scaf= 1 ;
while 1
    ...
    ScafR_Skip(ks-1)= dat.vstrands{Cyli}.skip(Current(2)+1 );
    
    ColRow(ks-1,: ) = [dat.vstrands{Cyli}.col ,dat.vstrands{Cyli}.row] ;
      ks=ks+1 ;
    ...
    if sum(Current==[-1,-1])==2
        ...
        nth_scaf= nth_scaf+1 ;
        if nth_scaf>size(Scaf_Start,1)
            break
        else
            Current=Scaf_Start(nth_scaf,:) ;
            ScafR_AllBase(ks-1,:)=Scaf_Start(nth_scaf,:);  
           
            ColRow(ks-1,:)= NumList(NumList(:,2)==Current(1),3:4);  % v3
...
ScafR_AllBase_Mindex=ScafR_AllBase;   %C5
...
for stapSSi= 1:    size(ColorCode,1)    % fill ds part
    ...
    BaseStart=ColorCode(stapSSi,2) ;   %C5
...
%---------find base pair
ScafR_AllBase_Mindex;  ScafR_Skip;
ScafR_AllBase_MindexWOSkip = ScafR_AllBase_Mindex(ScafR_Skip==0  ,:) ;

ScafR_cadnanoInd=  ScafR_AllBase_MindexWOSkip; 
ScafR_cadnanoInd(:,1) =NumList(  ScafR_cadnanoInd(:,1),2) ;

IndScafPaired = zeros(size(ScafR_AllBase_MindexWOSkip,1) ,1) ;
for bpi= 1:length(staplePlot)
   StapleBases =staplePlot{bpi}.UserData.IniPosition  ;
   [ia,ib]= ismember( StapleBases,  ScafR_AllBase_MindexWOSkip ,'rows') ;
   ib=ib(ib~=0) ;
    IndScafPaired(ib) =1;
end
    [L_bw,n_bw] = bwlabel(IndScafPaired==0) ;
    
    ssScafSeq = strings(n_bw,1) ;
    pSeq =p7249 ;  % hard code %*****************************************************
for bwj=1: n_bw
%     if bwj==80
%         sdfssfaff=3
%     end
   IndThisssDNA =  L_bw==bwj;
    ssScafSeq(bwj) = pSeq(IndThisssDNA);
    FF=find(IndThisssDNA) ;
%     fprintf(' \n')
%     fprintf('Start at %i  %i\n' , ScafR_AllBase_MindexWOSkip(FF(1),:)) ;
    fprintf('ssScaf%i n=%i = %s \n' ,bwj,length( pSeq(IndThisssDNA)), pSeq(IndThisssDNA)) ;
    fprintf('ssScaf%i Start= %i [%i] , End= %i [%i] \n' ,bwj,ScafR_cadnanoInd(FF(1),:),ScafR_cadnanoInd(FF(end),:)) ;
    fprintf('\n');
end

fileIDssScaf = fopen(strcat(PathName3,filesep,'ssScaf.txt'),'w');
for bwj=1: n_bw
    IndThisssDNA =  L_bw==bwj;
    fprintf(fileIDssScaf, '%s\n',pSeq(IndThisssDNA)) ;
end
fclose(fileIDssScaf);
...
%%
    function ExportJSON(src,evn,fH,pH , staplePlotAx2,BelongTransM ,popupH  ,editH,ax2,ScafR_AllBase_Mindex ,NumList,JSON_filename,PathName3 )
        ...
        file_name=strcat(   JSON_filename(1:end-5),'_moved2 '   ,'.json');
