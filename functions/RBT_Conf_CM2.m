...
for stapSSi= 1:    size(ColorCode,1)    % fill ds part
    ...
    stapAndSacf= intersect(find(ConvT_CylBase(:,1)==CylInd),find(ConvT_CylBase(:,2)==(BaseStart)));  % small change 01282019
...
while ~isempty(StapNoAss)
    for k=1:length(StapNoAss)
        ...
        if StapNoAss(k)+1<=size(ConvT_CylBase,1)
        Cand1BaseBC=  ConvT_CylBase( StapNoAss(k)+1,:);
        else
         Cand1BaseBC=[100000,100000];   
        end
