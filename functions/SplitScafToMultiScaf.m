function [OutputScafTangle, CutGood] =SplitScafToMultiScaf(src,evn)
...
if length(GetHyperB.ScafRouting) ==1   % start with one cycle case
    ...
    str = strcat('Current one long Scaf ~= ' , num2str(NBaseOri) ,' bp' ) ;
...
%     fprintf('NBaseOri~= %i  Mean: %i  Lower: %i Higher: %i .\n' ,NBaseOri ,round((NBaseOri/Npiece)) ,round((NBaseOri/Npiece)*(1-Acc/100)) ,round((NBaseOri/Npiece)*(1+Acc/100) )) ;
     Max_Number_Of_Trial  = 200; %------------mulitple trial until it succeed. Jan 17 2020
    for RunMoreTime = 1 : Max_Number_Of_Trial
...
%     RunMoreTime;
    ...
    if sum(allGood)== length(allGood)  % update if every cutting is good
        ...
        fprintf('Found the crossovers satisfying the ranges, update data .....\n') ;
    else
        %             figure ; ax=gca;
