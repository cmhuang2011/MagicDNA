...
% CentersVec2;  %unit:nm
%  Coeff=0.6 ; coeff2=0.34;
%   CentersVec3=CentersVec+Coeff*BVec+coeff2*NVec ;
%  [C,IA,IC] = uniquetol([CentersVec2;CentersVec3],0.0001,'ByRows',true,'OutputAllIndices',true ); 
%  LL_IA= cellfun('length',IA) ;
%  StackingCorrelate =zeros(sum(LL_IA ==2) ,2 ); cj=1;
%  Iamwroong=0;
%  for j=1:length(LL_IA)
%      if LL_IA(j)==2
%          StackingCorrelate(cj,:) =(IA{j})' ; cj=cj+1;
% %          checkSeq =(IA{j})'
% %          [
% %          SeqPair=saveallseq((IA{j})');
% %          if strcmp(SeqPair,'AT')||strcmp(SeqPair,'TA')||strcmp(SeqPair,'CG')||strcmp(SeqPair,'GC')
...
% %          else
% %              Iamwroong=Iamwroong+1 ;
% %               StackingCorrelate(cj-1,:) =[0,0 ] ;
% %          end
%          
%      end    
%  end
%  StackingCorrelate( sum(StackingCorrelate,2)==0 ,:)=[];
%  
%  StackingCorrelate=StackingCorrelate-1 ; % index difference between Matlab and python
% 
...
% end
% 
%     fprintf(fileID4,'%4.2f %4.2f %4.2f %i %i %i \n',  CentersVec2(1,1:3),bpi, mrDNA_stack(1,3) , 1)  ; s=1 ; 
%     for scafi=2:length(scafSeq)   
%         [u,v]=find(bptable==s);
%         if isempty(u)
%         bpi= -1 ;
%         elseif v==1
...
% %     end
% % 
% %     fprintf(fileID4,'%4.2f %4.2f %4.2f %i %i %i \n',  CentersVec2(s+1,1:3),bpi, -1 , s)  ; s=s+1 ; 
% 
%     
% %      fprintf(fileID4,'staplestart \n') ; 
%    
%     
%     for stpi2=1:length(GetHyperB.StapList3)
% %              fprintf(fileID4,'staplestart \n') ; 
...
%         
% %             fprintf(fileID2,'%u %c %d %u\n', stpi2+1, StapiSeq(1),-1, ss+3)  ;
%             for stapiline=2:length(StapiSeq)  
%                 [u,v]=find(bptable==s);
%                 if isempty(u)
%                     bpi= -1 ;
%                 elseif v==1
