function varargout=plotStaple_Helix( GetHyperB  )
% old: function pStapleH=plotStaple_Helix( GetHyperB  )
...
skipPattern2=39:60:MaxBase;
    ...
    pStapleH=cell(length(GetHyperB.StapList3),1) ;
      for stai=1:length(GetHyperB.StapList3)
          stai;
     ...
     AppedixStapHelR=zeros(10000,3);  % [Bundle , Cyl, Base];
        for edgeinSCR=1:2:size(StapAll,1)
             ...
             bundle=GetHyperB.RelateTable(GetHyperB.RelateTable(:,5)==C5Cyl,1)   ;%culti-section needs be cautious
% bundle
             Bundle=GetHyperB.containBundle{bundle};
             Cyl=GetHyperB.RelateTable(GetHyperB.RelateTable(:,5)==C5Cyl,2);   %C2 express
             
             if  Cyl~=-1   % overhang cylinders
                 ...
                 if ~isempty(GetHyperB.containBundle{bundle}.TransformMatrix2)  %if had done simulation ----------------
                   ...
                   Global_XYZ=Bundle.HelixRegardlessCylinder(1,1,InplaneXY,BasesArr,ThefSimilarDirCylder) ;   %  sdgdfgdgdfg
...
%              PartHelix2V2(:,1:2)=1.3*PartHelix2V2(:,1:2);       %---------debug
        ...
        pStapleH{stai}=plot3(  StapHelix(:,1),StapHelix(:,2),StapHelix(:,3),'-*','LineWidth',2);

      end

varargout{1} = pStapleH;
