function readSTEPFile(src,evn,allGraphics,ss_STEP,allbtn)
...
elseif isequal(allbtn.StartPoints ,src)  % use XYZ points as start
    ...
    readPoints(allbtn.StartPoints,[],allGraphics,ss_STEP) ;
...
sld1 = uicontrol('Style', 'slider','Parent',d,'Units','normalized',....
    'Min',0.5,'Max',4,'Value',1,'Position', [0.7 0.65 0.25 0.05]);    %  blow
...
btnExport2= uicontrol('Style','pushbutton','Parent',fH,'Units','normalized',...
    'Position',[0.8 0.05 0.1 0.1],  'String','Export Bundles');

InitTBP= sum(round(DistBtwTwoEnds/0.34))*4;
txtTotalBP = uicontrol('Parent',d,'Style','text','Units','normalized',...
    'Position',[0.9 0.05 0.1 0.1],   'String',strcat('Apx. Scaf = ',num2str(InitTBP))   );
...
% str
t = uitable(fH,'Units','normalized','Position',[0.7 0.15 0.25 0.45],....
...
btnLoadTable= uicontrol(fH,'Style','pushbutton','Units','normalized',...
    'Position',[0.9 0.83 0.05 0.05],  'String','LoadTableData');
btnLoadTable.Callback=@(src,evn)LoadTData(src,evn,t) ;

btnSaveTable= uicontrol(fH,'Style','pushbutton','Units','normalized',...
    'Position',[0.82 0.83 0.05 0.05],  'String','SaveTableData');
btnSaveTable.Callback=@(src,evn)saveTable(src,evn) ;
...
fprintf('Save current data in STEP_table_Data.mat. \n')
end
...
for k=1:size(src.Data,1)-1
    ...
    elseif strcmp(src.Data{k,3} , 'X_SQ')
